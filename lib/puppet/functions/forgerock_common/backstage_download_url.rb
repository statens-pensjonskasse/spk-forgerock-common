...
Puppet::Functions.create_function(:'forgerock_common::backstage_download_url') do
  ...
  def backstage_download_url(product_family_id, product_id, version, distribution, release_type = nil, platform = nil, platform_version = nil, os = nil, architecture = nil, include_system_ca_store = false)
    ...
    version_json = version(versions_from_minor_version(minor_version(minor_versions_from_product(product(products_from_family(family(families_from_data(data), product_family_id)), product_id)), dist_version)), version)

    dist = if product_family_id == 'am' && product_id == 'openam-web-policy-agents'
             distribution(distributions_from_architecture(architecture(architectures_from_os(os(oses_from_platform_version(platform_version(platform_versions_from_platform(platform(platforms_from_version(version_json), platform)), platform_version)), os)), architecture)), distribution)
           else
             distribution(distributions_from_release_type(release_type(release_types_from_version(version_json), release_type)), distribution)
           end

    artifact_id = dist['artifactId'] if dist
